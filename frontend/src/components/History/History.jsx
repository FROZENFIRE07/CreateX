...
const getGlassCard = (dark) => ({
    ...getGlass(dark), p: 6, rounded: '2xl', position: 'relative', overflow: 'hidden',
});
...
export default function History() {
    ...
    const glassCard = getGlassCard(dark);
                ...
                <VStack spacing={4} align="stretch">
                    <AnimatePresence mode="popLayout">
                        {filteredContents.map((item, idx) => {
                            const st = getStatusStyle(item.status || item.orchestrationStatus, T);
                            return (
                                <MotionBox
                                    key={item._id}
                                    custom={idx}
